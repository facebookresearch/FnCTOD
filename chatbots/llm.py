...
class LLM():
    def __init__(self, 
        ...
        self.openai_api = True if any([x in self.model_name for x in ["gpt-3.5", "gpt-4"]]) else False 
    ...
    def generate(self, 
        ...
        if self.openai_api: # the openai official api
            generations = self.model.generate(messages=prompt,
                                            functions=functions,
...
class chat_completion(object):
    ...
    def complete(self, 
        ...
        if self.template == "chatgpt":
            ...
            t1 = time.time()
            outputs = self.model.generate(prompt=messages,
                                          functions=functions,
                                          function_call=function_call,
                                          temperature=temperature,
                                          top_p=top_p,
            ...
            for output in outputs:
                if self.verbose: print("Before parsing:", output)
                parsed_output = self.conversation.get_response(output, function_call, required=required)
                if self.verbose: print("After parsing:", parsed_output)
