...
# 2.3
# data_path = 'data/MultiWOZ_2.3/'
# save_path = 'data/multi-woz-2.3-analysis/'
# save_path_exp = 'data/multi-woz-2.3-processed/'
# 2.4
# data_path = 'data/MultiWOZ_2.4/'
# save_path = 'data/multi-woz-2.4-analysis/'
# save_path_exp = 'data/multi-woz-2.4-processed/'
...
def analysis():
    ...
    if os.path.exists(data_path + data_file + ".zip"):
        ...
        # Extract reference numbers using regular expression
        ref_nos = list(set(re.findall(r'"reference": "(\w+)"', data)))
    ...
    for fn, dial in data.items():
        ...
        for turn in logs:
            if not turn["metadata"]:  # user's turn
                ...
                for (
                    ...
                    if record:
                        ...
                        turn_dict["metadata"][dom][
                            "semi"
                        ] = semi  # add that domain's semi
                compressed_raw_data[fn]["log"].append(
                    turn_dict
                )  # add to log the compressed turn_dict
            ...
            if fn in ["pmul2756.json", "pmul4958.json", "pmul3599.json"]:
                dial_type = "single"
            dial_domains = [
                dom for dom in domains if goals[dom]
            ]  # domains that are in demmand
            dom_str = ""
            for dom in dial_domains:
                if not dom_count.get(
                    dom + "_" + dial_type
                ):  # count each domain type, with single or multi considered
                    dom_count[dom + "_" + dial_type] = 1
                else:
                    dom_count[dom + "_" + dial_type] += 1
                if not dom_fnlist.get(
                    dom + "_" + dial_type
    ...
    with open(save_path + "domain_count.json", "w") as sf:
        ...
        other_count = [
            d for d in dom_count.items() if "multi" not in d[0] and "single" not in d[0]
        ]
