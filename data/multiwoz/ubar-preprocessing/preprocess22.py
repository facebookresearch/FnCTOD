...
def update_user_acts(domain, bs_list, user_act_dict):
    informable_slots = {
    ...
    if domain in informable_slots:
        if domain in user_act_dict:
            domain_bs = copy.deepcopy(user_act_dict[domain])
        else:
            domain_bs = {}
        for bs in bs_list:
            slot_name = bs[0].lower()
            slot_value = bs[1].lower()
            if slot_name == "price":
            ...
            if slot_name in informable_slots[domain]:
                if slot_name in domain_bs:
                    if slot_value not in domain_bs[slot_name]:
                        domain_bs[slot_name].append(slot_value)
                else:
                    domain_bs[slot_name] = [slot_value]
        user_act_dict[domain] = domain_bs
            ...
            for idx, entry in enumerate(
                ...
                for key, value in entry.items():  # key = slot
                    ...
                    tokenize_and_back = " ".join(
                        [token.text for token in nlp(value)]
                    ).strip()
...
class DataPreprocessor(object):
    def __init__(self):
        ...
        if os.path.exists(data_path + ".zip"):
            with zipfile.ZipFile(data_path + ".zip", "r") as archive:
                file_name = data_path.split("/")[-1]
                self.convlab_data = json.loads(
                    archive.open(file_name, "r").read().decode("utf-8").lower()
                )
        else:
            with open(data_path, "r", encoding="utf-8") as file:
                data = file.read().lower()  # Read and convert data to lowercase
                self.convlab_data = json.loads(data)
        ...
        self.delex_sg_valdict_path = (
            "data/multi-woz-2.2-processed/delex_single_valdict.json"
        )
        self.delex_mt_valdict_path = (
            "data/multi-woz-2.2-processed/delex_multi_valdict.json"
        )
    ...
    def delex_by_annotation(self, dial_turn):
        ...
        for s in span:
            ...
            try:
                for tid, token in enumerate(u_tokens):
                    if token == u[s[3] : s[4]]:
                        break
                u_tokens[tid] = "[value_" + slot + "]"
            except:
                print(f"text: {u}")
                print(f"index: {s[3]} - {s[4]}")
        u_delex = " ".join([t for t in u_tokens if t != ""])
    ...
    def get_delex_valdict(
                    ...
                    for slot, value in db_entry.items():
                        if slot not in skip_entry_type[domain]:
                            if type(value) is not str:
                                ...
                                if (
                                    value in entity_value_to_slot
                                    and entity_value_to_slot[value] != slot
                                ):
        ...
        for (
            amb_ent
        ) in ambiguous_entities:  # departure or destination? arrive time or leave time?
        ...
        with open(self.delex_sg_valdict_path, "w") as f:
            single_token_values = OrderedDict(
                sorted(
                    single_token_values.items(), key=lambda kv: len(kv[0]), reverse=True
                )
            )
        ...
        with open(self.delex_mt_valdict_path, "w") as f:
            multi_token_values = OrderedDict(
                sorted(
                    multi_token_values.items(), key=lambda kv: len(kv[0]), reverse=True
                )
            )
    ...
    def preprocess_main(self, save_path=None, is_test=False):
        ...
        for fn, raw_dial in tqdm(list(self.convlab_data.items())):
            ...
            compressed_goal = (
                {}
            )  # for every dialog, keep track the goal, domains, requests
            ...
            for turn_num, dial_turn in enumerate(raw_dial["log"]):
                ...
                dial_turn["text"] = " ".join(
                    [t.text for t in self.nlp(dial_turn["text"])]
                )
                if not dial_state:  # user
                    ...
                    if "dialog_act" in dial_turn:
                        user_acts = dial_turn["dialog_act"]
                        for domain_act, bs_list in user_acts.items():
                            domain, act = domain_act.split("-")
                            if act.lower() == "inform":
                                domain = domain.lower()
                                update_user_acts(domain, bs_list, user_act_dict)

                    if user_act_dict:
                        for d, acts in user_act_dict.items():
                            user_act += ["[" + d + "]"]
                            for slot in acts.keys():
                                user_act += [slot]
                    user_act = " ".join(user_act)
                    single_turn["user_act"] = user_act
                    ...
                    single_turn["nodelx_resp"] = " ".join(
                        clean_text(dial_turn["text"]).split()
                    )
                    ...
                    for domain, info_slots in constraint_dict.items():
                        if info_slots:
                            ...
                            if domain not in prev_constraint_dict:
                                ...
                                prev_constraint_dict[domain] != constraint_dict[domain]
                            ):
                                turn_dom_bs.append(domain)
                    ...
                    for act, params in dial_turn["dialog_act"].items():
                        ...
                        add_to_last = (
                            True
                            if a in ["request", "reqmore", "bye", "offerbook"]
                            else False
                        )
                        if add_to_last:
                    ...
                    if "user" in single_turn:
                        ...
                        for t in (
                            single_turn["user"].split()
                            + single_turn["resp"].split()
                            + constraints
                            + sys_act
                        ):
