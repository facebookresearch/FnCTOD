...
class MultiWozReader(_ReaderBase):
    def __init__(self, tokenizer, cfg, data_mode="train"):
        ...
        test_list = [
            l.strip().lower() for l in open(self.cfg.test_list, "r").readlines()
        ]
    ...
    def get_exp_domains(self, exp_domains, all_domains_list):
        if "hotel" in exp_domains:
            if "except" in exp_domains:
                # ['except', 'hotel']
                domains = [
                    d for d in all_domains_list if "hotel" not in d and "multi" not in d
                ]
            else:
                # ['hotel']
        ...
        if "train" in exp_domains:
            if "except" in exp_domains:
                # ['except', 'train']
                domains = [
                    d for d in all_domains_list if "train" not in d and "multi" not in d
                ]
            else:
                # ['train']
        ...
        if "attraction" in exp_domains:
            if "except" in exp_domains:
                ...
                domains = [
                    ...
                    if "attraction" not in d and "multi" not in d
                ]
            else:
                # ['attraction']
        ...
        if "restaurant" in exp_domains:
            if "except" in exp_domains:
                ...
                domains = [
                    d
                    for d in all_domains_list
                    if "restaurant" not in d and "multi" not in d
                ]
            else:
                # ['restaurant']
        ...
        if "taxi" in exp_domains:
            if "except" in exp_domains:
                # ['except', 'taxi']
                domains = [
                    d for d in all_domains_list if "taxi" not in d and "multi" not in d
                ]
            else:
                # ['taxi']
    ...
    def _load_data(self, save_temp=False):
        ...
        self.data = json.loads(
            open(self.cfg.data_path + self.cfg.data_file, "r", encoding="utf-8")
            .read()
            .lower()
        )
        self.train, self.dev, self.test = [], [], []
    ...
    def _get_encoded_data(self, fn, dial):
        ...
        for idx, t in enumerate(dial["log"]):  # tokenize to list of ids
            ...
            enc["nodelx_resp"] = self.tokenizer.convert_tokens_to_ids(
                self.tokenizer.tokenize("<sos_r> " + t["nodelx_resp"] + " <eos_r>")
            )
            ...
            db_pointer = self.bspan_to_DBpointer(
                t["constraint"], t["turn_domain"].split()
            )
        ...
        for idx, cons in enumerate(bspan):
            ...
            if "[" in cons:
                ...
                while (
                    ...
                    and vt != "<eos_b>"
                    and "[" not in vt
                    and vt not in ontology.get_slot
                ):
...
if __name__ == "__main__":
    ...
    for bspan in [
        "[taxi] destination golden house departure broughton house gallery arrive 19:30 [attraction] type museum name whipple museum of the history of science people 5 day monday",
        "[taxi] destination golden house departure broughton house gallery arrive 19:30 [attraction] type museum name whipple museum of the history of science people 5 day monday <eos_b>",
    ]:
