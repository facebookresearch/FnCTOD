    ...
    if domain_list == 1:  # only have one domain
        ...
        for idx in range(len(domain_list)):
            ...
            if idx == len(domain_list) - 1:  # last domain
                bs_text_snippet = (
                    curr_domain + " " + bs_text.split(curr_domain)[1].strip()
                )
                bsdx_text_snippet = (
                    curr_domain + " " + bsdx_text.split(curr_domain)[1].strip()
                )
            else:
                ...
                bs_text_snippet = (
                    curr_domain
                    + " "
                    + bs_text.split(curr_domain)[1].split(next_domain)[0]
                )
                bsdx_text_snippet = (
                    curr_domain
                    + " "
                    + bsdx_text.split(curr_domain)[1].split(next_domain)[0]
                )
...
def parse_bs_bsdx(bs_text, bsdx_text):
    ...
    return (
        " ".join(res_bs_text.split()).strip(),
        " ".join(res_bsdx_text.split()).strip(),
    )
...
def overall_parsing_bs_bsdx(bs_text, bsdx_text):
    ...
    for idx in range(len(bs_text_list)):
        one_bs_text, one_bsdx_text = parse_bs_bsdx(
            bs_text_list[idx], bsdx_text_list[idx]
        )
...
def process_dict(in_dict, tokenizer, special_token_list):
    ...
    bs_text_reform, bsdx_text_reform = overall_parsing_bs_bsdx(
        in_dict["bspn"], in_dict["bsdx"]
    )
...
def process_data_list(data_list, tokenizer, special_token_list):
    ...
    for item in data_list:
        ...
        try:
            one_res_list = [
                process_dict(sub_item, tokenizer, special_token_list)
...
if __name__ == "__main__":
    ...
    os.makedirs(r"../data/multi-woz-2.1-fine-processed/", exist_ok=True)
    ...
    out_f = r"../data/multi-woz-2.1-fine-processed/multiwoz-fine-processed-train.json"
    ...
    out_f = r"../data/multi-woz-2.1-fine-processed/multiwoz-fine-processed-dev.json"
    with open(out_f, "w") as outfile:
    ...
    out_f = r"../data/multi-woz-2.1-fine-processed/multiwoz-fine-processed-test.json"
    with open(out_f, "w") as outfile:
