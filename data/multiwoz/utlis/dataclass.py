...
class MultiWozData:
    def __init__(self, model_name, tokenizer, cfg, data_path_prefix, shuffle_mode='shuffle_session_level', 
        data_mode='train', data_version='2.0', use_db_as_input=True, cascaded=False, add_special_decoder_token=True):
        '''
        ...
        if data_mode in ['train', 'interact']:
            ...
            train_json_path = os.path.join(data_path_prefix, processed_data_path, 'multiwoz-fine-processed-train.json')
        ...
        if load_multiwoz_data:
            dev_json_path = os.path.join(data_path_prefix, processed_data_path, 'multiwoz-fine-processed-dev.json')
            ...
            test_json_path = os.path.join(data_path_prefix, processed_data_path, 'multiwoz-fine-processed-test.json')
    ...
    def convert_to_raw_data(self):
        ...
        return train_dials, dev_dials, test_dials, self.train_dial_id_list, self.dev_dial_id_list, self.test_dial_id_list
        ...
        for session in data:
            ...
            for turn_id in range(turn_num):
                ...
                if not self.cascaded:  
                    ...
                    nlg_input = self.nlg_prefix_id + [self.sos_context_token_id] + nlg_input[-900:] + [self.eos_context_token_id] + db_input + curr_aspn
            ...
            if ref_db:
                if not self.cascaded:
                    ...
                    if ref_act:
                        nlg_input_id_list = previous_context + curr_user_input
                        nlg_input_id_list = self.nlg_prefix_id + [self.sos_context_token_id] + nlg_input_id_list[-900:] \
                        + [self.eos_context_token_id] + db_input + one_instance['aspn']
                    else:
                        nlg_input_id_list = previous_context + curr_user_input
                        nlg_input_id_list = self.nlg_prefix_id + [self.sos_context_token_id] + nlg_input_id_list[-900:] \
                        + [self.eos_context_token_id] + db_input
    ...
    def build_all_evaluation_batch_list(self, ref_bs, ref_act, ref_db, input_contain_db, eva_batch_size, eva_mode):
        ...
        if eva_batch_size == -1: # not batch
            final_batch_list = [all_bs_input_id_list, all_da_input_id_list, all_nlg_input_id_list, all_parse_dict_list]
        else: # batch
