...
def process_session(sess_list):
    turn_num = len(sess_list)
    res_dict = {"dataset": "CamRes676", "dialogue_session": []}
    for idx in range(turn_num):
        if idx == 0:
...
if __name__ == "__main__":
    ...
    save_path = r"./data/pre-training_corpora/separate_datasets/CamRes676/"
    if os.path.exists(save_path):
        pass
    else:  # recursively construct directory
        os.makedirs(save_path, exist_ok=True)

    in_f = r"./data/pre-training_corpora/raw_data/CamRest676/CamRest676.json"
