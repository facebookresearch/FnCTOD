...
def zip_turn(prev_bs_dict, prev_bs_name_list, turn_list, domain):
    ...
    return usr_uttr, usr_bs, usr_bsdx, system_uttr, system_action_text, system_action_dict, bs_dict, bs_name_list
...
def process_session_list(session_list, domain):
    ...
    for idx in range(turn_num):
        ...
        one_usr_uttr, one_usr_bs, one_usr_bsdx, one_system_uttr, one_system_action, da_dict, \
...
if __name__ == '__main__':
    ...
    in_f = r'./data/pre-training_corpora/raw_data/e2e_dialog_challenge/data/taxi_all.tsv'
    domain = '[taxi]'
    taxi_res_list = process_file(in_f, domain)

    in_f = r'./data/pre-training_corpora/raw_data/e2e_dialog_challenge/data/movie_all.tsv'
    domain = '[movie]'
    ...
    save_kb_path = r'./data/pre-training_corpora/separate_datasets/MS_E2E/MS_E2E_DB.json'
                        ...
                        for act, act_values in acts.items():
                            if act in ontology[domain]["actions"]:
                                for act_value in act_values:
                                    if act_value not in ontology[domain]["actions"][act]:
                                        ontology[domain]["actions"][act].append(act_value)
                            else:
