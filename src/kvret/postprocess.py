...
from src.kvret.preprocess import all_domain, token_map
...
def load_schema():
    ...
    processed_data_path = f"./data/pre-training_corpora/processed_data/KVRET"
    if not os.path.exists(processed_data_path):
        os.makedirs(processed_data_path, exist_ok=True)

    schema_path = f"{processed_data_path}/normalized_schema.yml"

    raw_data_path = f"./data/pre-training_corpora/raw_data/kvret"
    entities_path = f"{raw_data_path}/kvret_entities.json"
    ...
    if not os.path.exists(schema_path):
        ...
        for domain in all_domain:
            ...
            for slot, slot_desc in token_map.items():
                if slot not in ["poi", "address"]:
                    possible_values = entities[slot][:10]
                else:
                    possible_values = []
...
def process_data(data, split, reader):
    ...
    for dial_idx, dial in enumerate(tqdm(data)):
        ...
        for turn_id, turn in enumerate(turns):
            ...
            processed_turn["dspn"] = "[car_assistant]"
            processed_turn["all_domains"] = ["[car_assistant]"]
...
def get_data_split(reader, n_train=-1, n_test=-1, return_list=False):
    ...
    preprocessed_data_path = f"./data/pre-training_corpora/separate_datasets/KVRET"
    processed_data_path = f"./data/pre-training_corpora/processed_data/KVRET"
    if not os.path.exists(processed_data_path):
    ...
    if not os.path.exists(f"{processed_data_path}/train_raw_dials.json"):
        with open(f"{preprocessed_data_path}/kvret_train.json", 'r') as file:
    ...
    if not os.path.exists(f"{processed_data_path}/test_raw_dials.json"):
        with open(f"{preprocessed_data_path}/kvret_test.json", 'r') as file:
...
def retrieve_demo(dialogs, schema, domains, n=100, max_turns=3, bs_da_ratios=[1.0, 0.0]):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='KVRET') #
