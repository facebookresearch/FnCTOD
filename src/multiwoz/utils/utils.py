    ...
    for domain, bs in goal.items():
        ...
        if bs:
            ...
            for slot_name, slot_value in bs.items():
                if reverse_renaming:
                    if domain in slot_renaming_reverse:
                        if slot_name in slot_renaming_reverse[domain]:
                            slot_name = slot_renaming_reverse[domain][slot_name]
                if slot_name in informable_slots[domain[1:-1]]:
                    if (ignore_none_bs and slot_value) or not ignore_none_bs:
    ...
    for i, d_idx in enumerate(domain_idx):
        ...
        for act, act_slots in domain_da.items():
            for act_slot in act_slots:
                dialog_act_list.append("-".join([domain, act, act_slot]))
    return dialog_act_list
...
if __name__ == "__main__":
    ...
    sent3 = paser_dict_to_aspn(dict1)
    print(sent3)
    exit()
